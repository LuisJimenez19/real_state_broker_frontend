...
const Pagination = ({
  ...
  const onclickHandle = (link: string | any) => {
  ...
  const setLabel = (label: string) => {
    if (label == "Next &raquo;") {
      return (
        <span>
          {" "}
          <ArrowRight className="text-xs mb-0.4 ml-0.4" />{" "}
        </span>
      );
      ...
      return (
        <span>
          {" "}
          <ArrowLeft className="text-xs mb-0.4 mr-0.4" />{" "}
        </span>
      );
    } else {
      return label;
        ...
        <div className="flex flex-wrap mt-8 justify-center">
          {links?.map((link, key) =>
            link.url === null ? (
              <div
                key={key}
                className="first:ml-0 text-xs font-bold mx-2 flex w-8 h-8 p-0 rounded-full items-center justify-center leading-tight relative border border-solid border-gray-500 bg-white text-gray-500"
              >
                {setLabel(link.label)}
              </div>
            ) : (
              <button
                key={key}
                onClick={() => onclickHandle(link?.url)}
